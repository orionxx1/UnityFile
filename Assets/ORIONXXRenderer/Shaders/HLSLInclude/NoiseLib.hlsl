...
// Perlin Noise     梯度噪声（Gradient Noise）
...
float3 VoronoiID(float2 uv, float randomness) 
{
    float2 iuv = floor(uv);
    float2 fuv = frac(uv);

    float minDist = 1.0;  // 最小距离
    float3 minColor = float3(0, 0, 0); // 最近细胞的颜色
    ...
    for (int y = -1; y <= 1; y++) 
    {
        for (int x = -1; x <= 1; x++) 
        {
            ...
            float3 rand = hash2to3(iuv + neighbor); // 随机特征点+颜色
            float2 p = lerp(neighbor, neighbor + rand.xy, randomness); // 直接在此处计算point
            float2 diff = p - fuv;
            float dist = length(diff); // Euclidean距离

            if (dist < minDist) 
            {
                minDist = dist;
                minColor = rand; // 使用哈希值作为颜色
            }
        }
    }
    return  saturate(minColor);
}
