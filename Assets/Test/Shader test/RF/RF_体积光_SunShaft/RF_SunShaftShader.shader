    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag_m(Varyings IN) : SV_Target
            {
                ...
                ColorRGB = ColorRGB * step(depth,0.0) * region ;
        ...
        Pass
        {
            ...
            half4 frag_m(Varyings IN) : SV_Target
            {
                ...
                for (int j = 0; j < _StepSum; j++)
		        {
                    ...
                    tmpColor = tmpColor * Luminance(tmpColor);
                    half w =  saturate(tmpColor - color);
		        	color += tmpColor * sqrt(JJJ/_StepSum)* lerp(1, 0.5,w);
		        	uv += blurVector * (1 + seed * _SeedIntensity);
                    JJJ -= 1;
		        }
                color = color * angle * _LightIntensity * _LightColor.rgb;
