...
#else   //金属工作流
    half oneMinusReflectivity = OneMinusReflectivityMetallic(metallic); // 当前的反射率  也就是漫反射比例 （0.04 - 0.96）
    half reflectivity = half(1.0) - oneMinusReflectivity;               // 镜面反射率
    half3 brdfDiffuse = albedo * oneMinusReflectivity;                  // 漫反射颜色
    half3 brdfSpecular = lerp(kDieletricSpec.rgb, albedo, metallic);    // 高光颜色 （0.04 - 基础色）
#endif
...
// 计算Minimalist Cook-Torrance BRDF的标量镜面项
// 注意:需要乘以反射系数f0，即镜面颜色来完成
