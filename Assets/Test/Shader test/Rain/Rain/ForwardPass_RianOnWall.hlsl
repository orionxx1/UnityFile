...
#include "Assets/Test/Shader test/Rain/Rain/Lighting.hlsl"
#include "Assets/ORIONXXRenderer/Shaders/HLSLInclude/NoiseLib.hlsl"
#include "Assets/Test/Shader test/Rain/Rain/Rain.hlsl"
...
void Fragment( Varyings input, out half4 outColor : SV_Target0)
{
    ...
    half3 SplatsEngine = SplatsEngineF( input.uv, NoiseColor.rg, _SplatsSpeed*_Time.y, _SplatsSize, _SplatsNoise, _SplatOffest);
    half2 SplatsMask = SplatsMaskF( SplatsEngine, _SplatsIntensity, _SplatsScale);

    half  DropletID;
    half2 DropletUV;
    half2 DropletSizeData;
    half  DropletLife;
    DropletEngineF( DropletID, DropletUV, DropletSizeData, DropletLife, input.uv,
                    _DropletsSize, _Time.y*_DropletSpeed, _DropletMaxLength, _DropletMinLength);
    half3 DropletMask = DropletMaskF( NoiseColor.b, DropletUV, DropletSizeData, DropletID , 
                                      DropletLife, _DropletsIntensity, _DropletsNoise );

    half3 RainMaskData;
    RainMaskData.r = saturate(SplatsMask.r + DropletMask.b + _Wetness);      // 光滑度
    RainMaskData.g = saturate(SplatsMask.g*10 + DropletMask.r);              // 法线混合遮罩
    RainMaskData.b = SplatsMask.g;

    half HeightData = SplatsMask.g*_SplatsHeight*0.1 + DropletMask.g*_DropletHeight;  //高度
    uv = uv + HeightData*(DropletUV-0.5);                         // uv调整

    // ——————————————————————————————
    SurfaceData surfaceData;
    InitializeStandardLitSurfaceData(uv, surfaceData);
    surfaceData.smoothness =  lerp( surfaceData.smoothness, 0.95, RainMaskData.r);      // 修改粗糙度

    // 求出渲染PBR所需的数据  —— 世界空间法线，切线，视线   阴影坐标 ...
    InputData inputData;    
    InitializeInputData(input, surfaceData.normalTS, inputData);

    inputData.normalWS = lerp(inputData.normalWS, input.normalWS ,RainMaskData.g);
    inputData.normalWS = height2BumpWS(inputData.positionWS, inputData.normalWS, HeightData, 
                                       1, 1, 1, 1 );
    ...
    // color.rgb = SplatsMask.r;    // 粗糙度 
    // color = SplatsEngine.r;      //size
    // color = SplatsEngine.g;     //life
    // color = SplatsEngine.b;     //id
