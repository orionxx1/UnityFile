...
half3 SampleSH(half3 normalWS)
{
    ...
    real4 SHCoefficients[7];            // Unity 填充的球谐系数
...
half3 GlossyEnvironmentReflection(half3 reflectVector, float3 positionWS, half perceptualRoughness, half occlusion, float2 normalizedScreenSpaceUV)
{
        ...
        half3 irradiance;
            ...
            half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness);           // 根据粗糙的获取不同的mip层级 
            half4 encodedIrradiance = half4(SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, reflectVector, mip));
    
            irradiance = DecodeHDREnvironment(encodedIrradiance, unity_SpecCube0_HDR);  // HDR贴图，还需要进行HDR转换
...
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);      //视线在物体上反射向量
    half NoV = saturate(dot(normalWS, viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);                             //菲尼尔部分

    //间接光照
    half3 indirectDiffuse = bakedGI;        //间接漫反射光照
    //间接镜面反射光照
...
half3 GlobalIllumination(BRDFData brdfData,
                        ...
                        half3 viewDirectionWS)  
{
    return GlobalIllumination(brdfData, bakedGI, occlusion, positionWS, normalWS, viewDirectionWS, float2(0.0f, 0.0f));
}
#endif
