Shader "MyShader/NPR "
{
    ...
    {   
        ...
        [Tex(Group1)]_MaskMap("Mask贴图", 2D) = "white" {}        // ILM贴图 R=金属度     G=未知     B=AO    Alpha=光泽度 
        [Tex(Group1)]_NormalMap("法线贴图", 2D) = "bump" {}   
        [Tex(Group1)]_EmissionMap("自发光贴图", 2D) = "black" {}   

        [Sub(Group1)] _SmoothnessDef("默认光滑度", Range(0.0, 1.0)) = 0.5    
        [Sub(Group1)] _Smoothness("光滑度系数", Range(0.0, 1.0)) = 0.5
        [Sub(Group1)] _MetallicDef("默认金属度", Range(0.0, 1.0)) = 0.0         
        [Sub(Group1)] _Metallic("金属度系数", Range(0.0, 1.0)) = 0.0
        [Sub(Group1)] _OcclusionScale("AO系数",Range(0.0,2.0)) = 0.5   
        [Sub(Group1)] _NormalScale("法线强度", Range(0.0, 2.0)) = 1.0     
        [Sub(Group1)] _EmissionScale("自发光强度",Range(0.0,2.0)) = 0.5   
        ...
        [Sub(Group2)] _FrontColor("自定义亮面颜色", Color) = (1,1,1,1)        
        [Sub(Group2)] _ShadowColor("自定义阴影颜色", Color) = (1,1,1,0)        
        [Sub(Group2)] _ShadowSmooth("阴影平滑系数", Range(0.0, 4.0)) = 0    

        [Title(Ramp)]
        [Tex(Group2)] _RampMap("Ramp贴图", 2D) = "white" {}                
        [Sub(Group2)] _RampScale("Ramp系数", Range(0.0, 1.0)) = 0.5        
        
        [Title(MatCap)]
        [Tex(Group2)] _MatCap("MatCap", 2D) = "white" {}                        
        [Sub(Group2)] _MatCapColor("MatCap染色", Color) = (1,1,1,0)             
        [Sub(Group2)] _ScaleOfMult("金属度影响系数", Range(0.0, 1.0)) = 0.0        
        [Title(Blend_Add_Mulit_Overlay )]
        [Sub(Group2)] _MatCapMode ("MatCap混合模式", Range(0.0, 3.0)) = 0        //  0=混合 1=相加 2=相乘 3=叠加
        [Sub(Group2)] _MatCapIntensity("MatCap强度", Range(0.0, 5.0)) = 0   
        [Sub(Group2)] _MatCapScale("MatCap系数", Range(0.0, 1.0)) = 0.0         
        ...
        // 描边
        [Main(M, _, on, off)] _M ("描边设置", float) = 0
        ...
        // 深度边缘高光   深度阴影
        [Main(Rim, _, on, off)] _Rim ("边缘高光", float) = 0
        [Sub(Rim)] _RimOffset ("RimOffset", Range(0.0, 30.0)) = 1 
        [Sub(Rim)] _RimScale ("RimScale", Range(0.0, 10.0)) = 1 
        [Sub(Rim)] _DepthShadowScale ("DepthShadowScale", Range(0.0, 10.0)) = 1 
        ...
        // 混合设置
        [Main(Preset, _, on, off)] _PresetGroup ("混合设置", float) = 0
