...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Shadows.hlsl"     //实现阴影部分
...
struct Light
{
    half3   direction;              // 方向
    half3   color;                  // 颜色
    float   distanceAttenuation;    // 距离衰减  full-float precision required on some platforms   在某些平台上需要全浮动精度
...
//获取主光源并计算其阴影衰减和光照效果，同时还引入了额外的AO和光照 Cookie 处理
Light GetMainLight(float4 shadowCoord)
{
    Light light = GetMainLight();        // 获取主光源的基本信息
    light.shadowAttenuation = MainLightRealtimeShadow(shadowCoord);      // 计算实时阴影衰减
    return light;
}
...
Light GetMainLight(float4 shadowCoord, float3 positionWS, half4 shadowMask)
{
    ...
    // 启用Cookie
        ...
        real3 cookieColor = SampleMainLightCookie(positionWS);       //采样光照Cookie
        light.color *= cookieColor;     // 修改光照颜色，添加Cookie效果  —— 用于给光源添加一些图案或纹理效果
    #endif
...
{
    // 这行调用了第二个 GetMainLight 函数      获取主光源并计算阴影衰减
    Light light = GetMainLight(inputData.shadowCoord, inputData.positionWS, shadowMask);

    // 是否启用AO  同时  表面类型是否为不透明
    #if defined(_SCREEN_SPACE_OCCLUSION) && !defined(_SURFACE_TYPE_TRANSPARENT)
    // 是否启用AO
    if (IsLightingFeatureEnabled(DEBUGLIGHTINGFEATUREFLAGS_AMBIENT_OCCLUSION))
    {
        light.color *= aoFactor.directAmbientOcclusion;     // 乘上环境光遮蔽因素
    }
    #endif
